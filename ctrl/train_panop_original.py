...
from ctrl.train_panop_utils import pool_semantic_feat_based_on_inst_pred, compute_hungarian_based_loss # TODO: put this on the top
from time import perf_counter
from torch.nn import functional as F
# from ctrl.evaluate_panop import eval_panoptic
from ctrl.evaluate_panop_sep25 import eval_panoptic

def train_model_ori(cfg, model, discriminator, discriminator2nd, resume_iteration,
    ...
    hungarian_based_triplet_loss = None
    ...
    if cfg.ACTIVATE_DANDA_MEMORY_MODULE:
        hungarian_based_triplet_loss = criterion_dict['hungarian_based_triplet_loss']

    inst_seg_source = None
    inst_seg_target = None

    if cfg.APPROACH_TYPE == 'DANDA':
        from ctrl.sup_con_loss import SupConLoss
        criterion_sup_con = SupConLoss(temperature=0.7)
    ...
    elif cfg.PANOPTIC_DEEPLAB_DATA_AUG_RANDOM_CROP:
    ...
    if not cfg.TRAIN_ONLY_SOURCE:
        if cfg.PANOPTIC_DEEPLAB_DATA_AUG_RANDOM_CROP:
    ...
    if cfg.ACTIVATE_DANDA_MEMORY_MODULE:
        loss_meter_dict['hung_loss'] = AverageMeter()
    ...
    loss_adv_gen2nd = 0.0
    loss_d2nd = 0
    # max_iter = 100 # TODO : remark
    # train loop

    if ACTIVATE_PERF_COUNTER:
        perf_time = {}
        perf_time['src_data'] = []
    ...
    try:
        for i_iter in range(start_iter, max_iter):
            ...
            if ACTIVATE_PERF_COUNTER:
                ...
                source_dataloader_time = perf_counter() - start_time2
            ...
            pred = model(images_source, domain_label=source_label)
            # pred = model(images_source)
            psf_source = pred['semantic'] # predicted semanitc feature
            pred = get_module(model, cfg.DISTRIBUTED).upsample_predictions(pred, input_shape_source)

            if ACTIVATE_PERF_COUNTER:
                torch.cuda.synchronize(DEVICE)
                forward_pass_time = perf_counter() - start_time2
            ...
            if ACTIVATE_PERF_COUNTER:
                torch.cuda.synchronize(DEVICE)
                supervised_loss_time = perf_counter() - start_time2
            ...
            if cfg.APPROACH_TYPE == 'DANDA':
                ...
                instance_source, center_source = inst_seg_source(semantic_pred_src,
                                                                 center_pred_source.detach(),
                                                                 offset_pred_source.detach())
            ...
            # target data loader
            if cfg.ACTIVATE_DANDA_MEMORY_MODULE or not cfg.TRAIN_ONLY_SOURCE and cfg.ENABLE_DISCRIMINATOR:
                try:
                ...
                if ACTIVATE_PERF_COUNTER:
                    torch.cuda.synchronize(DEVICE)
                    target_dataloader_time = perf_counter() - start_time2
                ...
                psf_target = pred_target['semantic']  # predicted semanitc feature
            ...
            if cfg.ACTIVATE_DANDA_MEMORY_MODULE:
                print()
                assert psf_source.size(1) == num_classes, 'error in code!'
                con_labels = torch.arange(0, psf_source.size(1))
                con_labels = con_labels.repeat(2).long()
                # psf_source  (B x 16 x H' x W')
                # psf_target    (B x 16 x H'' x W'')
                psf_source = psf_source.squeeze()

                # resize the target feature same as the source feature sptial dim
                psf_target = F.interpolate(psf_target, size=(psf_source.shape[1], psf_source.shape[2]), mode='bilinear', align_corners=True)

                psf_target = psf_target.squeeze()
                psf_source = psf_source.view(num_classes, -1) # 16 x (H*W)
                psf_target = psf_target.view(num_classes, -1) # 16 x (H*W)
                psf = torch.cat((psf_source, psf_target), dim=0)
                psf = psf.unsqueeze(dim=1)

                for i in range(15):
                    if torch.all(psf[i].eq(psf[i+16])):
                        print('features for class-id {} from source and target are equal'.format(i))

                hung_loss = criterion_sup_con(psf, con_labels)

                # # pull semantic features based on instance predictions
                # inst_feat_src = pool_semantic_feat_based_on_inst_pred(cfg, instance_source, semantic_pred_source, center_pred_source, depth_pred_source, DEVICE, i_iter)
                # inst_feat_tar = pool_semantic_feat_based_on_inst_pred(cfg, instance_target, semantic_pred_target, center_pred_target, depth_pred_target, DEVICE, i_iter)
                # # do the hungarian matching and compute the triplet loss
                # hung_loss_src = compute_hungarian_based_loss(cfg, DEVICE, inst_feat_src, model.memory_weights, hungarian_based_triplet_loss, i_iter)
                # hung_loss_tar = compute_hungarian_based_loss(cfg, DEVICE, inst_feat_tar, model.memory_weights, hungarian_based_triplet_loss, i_iter)
                # hung_loss = (hung_loss_src + hung_loss_tar) * cfg.TRAIN.LAMBDA_HUNGARIAN_BASED_LOSS
                # loss_meter_dict['hung_loss'].update(hung_loss.detach().cpu().item(), batch_size)


            # summing up all supervised and triplet losses
            loss = None
            if not cfg.TRAIN.TRAIN_INSTANCE_BRANCH and not cfg.TRAIN.TRAIN_DEPTH_BRANCH:
                loss = loss_semantic
            elif not cfg.TRAIN.TRAIN_INSTANCE_BRANCH and cfg.TRAIN.TRAIN_DEPTH_BRANCH:
            ...
            elif cfg.ACTIVATE_DANDA_MEMORY_MODULE and not cfg.TRAIN.TRAIN_DEPTH_BRANCH:
                loss = loss_semantic + loss_center + loss_offset + hung_loss
            elif cfg.ACTIVATE_DANDA_MEMORY_MODULE and cfg.TRAIN.TRAIN_DEPTH_BRANCH:
                loss = loss_semantic + loss_center + loss_offset + loss_depth + hung_loss

            if ACTIVATE_PERF_COUNTER:
                torch.cuda.synchronize(DEVICE)
                unlabeled_train_time = perf_counter() - start_time2
            ...
            if not cfg.TRAIN_ONLY_SOURCE and cfg.ENABLE_DISCRIMINATOR:
                ...
                #                                 center_pred_target, depth_pred_target.detach(), DEVICE, model.memory_weights)
                # else:
                ...
                #                                           center_pred_source, depth_pred_source, DEVICE,  model.memory_weights)
                # else:
                ...
                #                                           semantic_pred_target, center_pred_target, depth_pred_target, DEVICE,  model.memory_weights)
                # else:
                d_out, d_out2nd = disc_fwd_pass(cfg, discriminator, discriminator2nd, semantic_pred_target, center_pred=center_pred_target,
                                  offset_pred=offset_pred_target, depth_pred=depth_pred_target, mode=cfg.ADV_FEATURE_MODE)
            ...
            if ACTIVATE_PERF_COUNTER:
                torch.cuda.synchronize(DEVICE)
                loss_bacward_time = perf_counter() - start_time2
            ...
            if cfg.ACTIVATE_PANOPTIC_EVAL:
                if (not cfg.DISTRIBUTED and ((i_iter + 1) % cfg.TRAIN.EVAL_EVERY == 0)) \
                    ...
                    panop_eval_writer = SummaryWriter(log_dir=panop_eval_folder_dict['tensorboard'])
                    logger.info('')
            # FREE UP GPU MEMORY FOR THE VARIABLE SIZED INSTANCE BATCHES FOR THE TRIPLAET LOSS
            torch.cuda.empty_cache()

        if ACTIVATE_PERF_COUNTER == True:
            import numpy as np
