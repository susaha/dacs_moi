...
_CITYSCAPES_PANOPTIC_TRAIN_ID_TO_EVAL_ID_16 = [
    ...
    33, # bicycle
    0]
_CITYSCAPES_THING_LIST_16 = [10, 11, 12, 13, 14, 15] # [person, rider, car, bus, motorcycle, bicycle]
...
class CityscapesDataSet(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataloading_mode == 'panoptic':
            ...
            if self.cfg.DACS_RANDOM_CROP and self.is_train:
                image, label_panop, semseg_label = self.dacs_random_crop(image, label_panop, semseg_label, train_mode=self.is_train)
                image = image.transpose((2, 0, 1))
            ...
            #     np.save(f, label_panop_dict['semantic_instance'])
            #     np.save(f, label_panop_dict['foreground'])
            #     np.save(f, label_panop_dict['center'])
            #     np.save(f, label_panop_dict['center_points'])
            #     np.save(f, label_panop_dict['offset'])
            #     np.save(f, label_panop_dict['semantic_weights'])
