...
from ctrl.dacs_old.data.build_aug import dacs_build_aug, dacs_build_augV3, dacs_build_augV2
from ctrl.utils.synthia_cityscapes_gt_visualization import visualize_panoptic_gt, visualize_depth_gt, visualize_panoptic_gt_calss_wise
...
class CityscapesDataSet(data.Dataset):
    ...
    def __init__(self, root, list_path, set='val', max_iters=None, crop_size=(512, 1024),
        ...
        for ann in dataset['annotations']:
            ...
            depth_label_file = os.path.join(self.root, 'depth', self.set, name.split('_')[0], name.replace('_gtFine_panoptic.png', '_depth_stereoscopic.mat')) # TODO: correct the file name
            segments_info = ann['segments_info']
            self.files.append((img_file, label_file, panop_label_file, depth_label_file, segments_info, name))
        ...
        if self.set == 'train' and self.cfg.DACS_RANDOM_CROP:
            ...
            if self.gen_panop_labels:
                ...
                self.dacs_random_crop = dacs_build_augV3(transform_param, is_train=True, dataset='cityscapes')
    ...
    def __getitem__(self, index):
        if self.set == 'train':
            img_file, label_file, panop_label_file, depth_label_file, segment_info, name = self.files[index]
            ...
            #  you only want to load the panoptic gt labels for Oracle training on the Cityscapes,
            #  so for most of the exp during training this part is False
            if self.gen_panop_labels:

                # load the cityscapes pseudo depth labels provided by CORDA
                if self.cfg.LOAD_CITYSCAPES_CORDA_DEPTH_LABELS:
                    depth = io.loadmat(depth_label_file)["depth_map"]
                    depth = np.clip(depth, 0., 655.35)
                    depth[depth < 0.1] = 655.35
                    depth = 655.36 / (depth + 0.01)
                    depth = cv2.resize(depth, tuple(self.labels_size), interpolation=cv2.INTER_NEAREST)
                else:
                    # Dummpy depth not used
                    depth = 0. * np.array(semseg_label.copy(), dtype=np.float32)
                ...
                # visual
                # if self.cfg.DEBUG:
                #     outpath = '/home/suman/temp_123/visual_nov18/before_crop'
                #     visualize_panoptic_gt(dataset='cityscapes', mode='train', semantic=semseg_label, center=center, center_weights=center_w,
                #                           offset=offset, offset_weights=offset_w, out_path=outpath, fname=name)
                ...
                if self.cfg.DACS_RANDOM_CROP:
                    ...
                    image, semseg_label, center, center_w, offset, offset_w, depth = \
                        self.dacs_random_crop(image, semseg_label, center, center_w, offset, offset_w, depth=depth, use_depth=True, train_mode=True)
                    image = image.transpose((2, 0, 1))
                    center = center.transpose((2, 0, 1))
                    offset = offset.transpose((2, 0, 1))
                # visual
                # if self.cfg.DEBUG:
                #     outpath = '/home/suman/temp_123/visual_nov18/after_crop'
                #     visualize_panoptic_gt(dataset='cityscapes',  mode='train', semantic=semseg_label, center=center, center_weights=center_w,
                #                           offset=offset, offset_weights=offset_w, out_path=outpath, fname=name)
                label_panop_dict['semantic'] = torch.as_tensor(semseg_label.astype('long'))  # semanitc gt label

                return image, semseg_label, center.copy(), center_w.copy(), offset.copy(), offset_w.copy(), depth.copy(), np.array(image.shape), name
                ...
                # if self.cfg.DEBUG:
                #     outpath = '/home/suman/temp_123/visual_nov18/before_crop'
                #     visualize_depth_gt(dataset='cityscapes', mode='train', semantic=semseg_label, out_path=outpath, fname=name, depth=depth, img=image.copy(), img_mean=self.mean)

                if self.cfg.DACS_RANDOM_CROP:
                    image = image.transpose((1, 2, 0))
                    image, semseg_label, depth = self.dacs_random_crop(image, semseg_label, depth=depth, use_depth=True, train_mode=True)
                    image = image.transpose((2, 0, 1))
                ...
                return image, semseg_label, depth.copy(), np.array(image.shape), name

        else:
            img_file, label_file, panop_label_file, _, segment_info, name = self.files[index]
            # self.files.append((img_file, label_file, panop_label_file, depth_label_file, segments_info, name))
            image = self.get_image(img_file)
            image = self.preprocess(image).copy()
            semseg_label = self.get_labels(label_file)
