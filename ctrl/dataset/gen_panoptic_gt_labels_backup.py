...
class PanopticTargetGenerator_for_dumping_nparray(object):
    """
    ...
    def __call__(self, panoptic, segments, VIS=False):
        ...
        # create a dict to access tensor indices for each thing class
        dcid = {}
        cid_count = 0
        ...
        center_weights_cls_wise = np.zeros((num_thing_classes, panoptic.shape[0], panoptic.shape[1]), dtype=np.uint8)
        ...
        for seg in segments:
            ...
            if not seg['iscrowd']:
                ...
                if cat_id in self.thing_list:
                    center_weights_cls_wise[dcid[cat_id]][panoptic == seg["id"]] = 1
            ...
            if cat_id in self.thing_list:
                ...
                offset is of shape 2,H,W, offset_y_index is used to access the elements of offset tensor along first channel that is offset[0,:]
                all elements of offset_y_index[0] are 0s, so it access the first channel of offset, ie. offset[0,:]
                offset_y_index[1] has the y coordinate's positional index for as per the mask_index[0], that means in a HxW plane,
        ...
        return dict(
            center_cls_wise=center_cls_wise,
            offset_cls_wise=offset_cls_wise,
            center_weights_cls_wise=center_weights_cls_wise,
            offset_weights_cls_wise=offset_weights_cls_wise,
        )

        # cityscapes valset: we need these for evaluating on cityscapes
        # return dict(
...
class PanopticTargetGenerator(object):
    ...
    def __init__(self, logger, ignore_label, rgb2id, thing_list, sigma=8, ignore_stuff_in_offset=False,
        ...
        self.logger = logger
        self.logger.info('ctrl/dataset/gen_panoptic_gt_labels.py --> class PanopticTargetGenerator_Original() : __init__()')
