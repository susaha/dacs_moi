...
import os
from ctrl.dataset.gen_panoptic_gt_labels import PanopticTargetGeneratorForSynthiaClassWise, PanopticTargetGeneratorForSynthia
...
class SYNTHIADataSetDepth(data.Dataset):
    def __init__(
        ...
        if self.panop_label_type == 'class_wise':
            ...
            self.target_transform = PanopticTargetGeneratorForSynthia(self.logger, self.ignore_label, self.rgb2id, self.synthia_thing_list,
                                                                       sigma=8, ignore_stuff_in_offset=self.ignore_stuff_in_offset,
                                                                       small_instance_area=self.small_instance_area,
                                                                       small_instance_weight=self.small_instance_weight)
