...
class SYNTHIADataSetDepth(BaseDataset):
    def __init__(
        ...
        self.logger.info('ctrl/dataset/synthia_panop_dacs.py -->  class SYNTHIADataSetDepth() : __init__()')
    ...
    def __getitem__(self, index):
        ...
        # *** read pickle panoptic label files ***
        # panop_label_fname = os.path.join(self.root, "pickle_panoptic_labels", name.replace(".png", ".pkl"))
        # panop_label_file = open(panop_label_fname, 'rb')
        # panop_label_dict = np.load(panop_label_file, allow_pickle=True)
        # center = panop_label_dict['center']
        # center_w = panop_label_dict['center_weights']
        # offset = panop_label_dict['offset']
        # offset_w = panop_label_dict['offset_weights']

        # *** read numpy panoptic label files ***
        # panop_label_fname = os.path.join(self.root, "temp/numpy_panoptic_labels_cls_wise", name.replace(".png", ".npy"))
        # with open(panop_label_fname, 'rb') as f:
        #     center = np.load(f)     # 6,H,W
        #     center_w = np.load(f)   # 6,H,W
        #     offset = np.load(f)     # 6,2,H,W
        #     offset_w = np.load(f)   # 6,H,W

        panop_label_fname = os.path.join(self.root, "numpy_panoptic_labels", name.replace(".png", ".npy"))
        with open(panop_label_fname, 'rb') as f:
            center = np.load(f)       # 1,H,W
            center_w = np.load(f)     #   H,W
            offset = np.load(f)       # 2,H,W
            offset_w = np.load(f)     #   H,W

        # if self.cfg.DEBUG:
        #     center = np.resize(center, (1, 256,256))
        #     center_w = np.resize(center_w, (256, 256))
        #     offset = np.resize(offset, (2, 256, 256))
        #     offset_w = np.resize(offset_w, (256, 256))

        # visual
        # if self.cfg.DEBUG:
        #     outpath = '/home/suman/temp_123/visual_nov_7/before_crop'
        ...
        #     offset_w = offset_w.transpose((2, 0, 1))
            ...
            # visual
            # if self.cfg.DEBUG:
            #     outpath = '/home/suman/temp_123/visual_nov_7/after_crop'
            #     visualize_panoptic_gt_calss_wise(dataset='synthia', center_cw=center, center_w_cw=center_w, offset_cw=offset, offset_w_cw=offset_w, out_path=outpath, fname=name)
