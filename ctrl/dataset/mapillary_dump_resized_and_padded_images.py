...
import json
from ctrl.dacs_old.data.build_aug import dacs_build_aug, dacs_build_augV5
...
class MapillaryDataSet(data.Dataset):
    ...
    def __init__(self, root, list_path, set='val', max_iters=None, crop_size=(512, 1024),
                 ...
                 gen_panop_labels=False, scale_label=True, viz_outpath=None,
                 img_out_path=None, label_out_path=None):
        ...
        self.files = []
        json_filename = '{}/{}/panoptic_mapped_to_cityscapes_ids/pngs/panoptic.json'.format(self.root, self.set)
        dataset = json.load(open(json_filename))
        for ann in dataset['annotations']:
            name = ann['file_name']
            img_file = '{}/{}/images/{}'.format(self.root, self.set, name.replace('png', 'jpg'))
            panop_label_file = '{}/{}/panoptic_mapped_to_cityscapes_ids/pngs/panoptic/{}'.format(self.root, self.set, name)
        ...
        if self.set == 'train' and self.cfg.DACS_RANDOM_CROP:
            ...
            if self.gen_panop_labels:
                ...
                self.dacs_random_crop = dacs_build_aug(transform_param, is_train=True, dataset='mapillary')
    ...
    def __getitem__(self, index):
        ...
        if self.set == 'train':
            ...
            # here you need to convert the numpy image to PIL image and save as png
            # here you need to convert the numpy label_panop to PIL image and save as png
            pil_image = Image.fromarray(image.astype(dtype=np.uint8))
            out_fname = '%s/%s.png' % (self.img_out_path, name)
            with open(out_fname, mode='wb') as f:
                pil_image.save(f, 'JPEG')
            pil_image = Image.fromarray(label_panop.astype(dtype=np.uint8))
            out_fname = '%s/%s.png' % (self.label_out_path, name)
            with open(out_fname, mode='wb') as f:
            ...
            label_panop = Image.open(panop_label_file)
            label_panop = pad_with_fixed_AS(self.image_size[0] / self.image_size[1], label_panop, fill_value=self.bg_idx, is_label=True)
    ...
    def visualize_all_during_val(self, name, image, semantic, center, center_w, offset, offset_w, train_mode, foreground, semantic_weights):
        save_dir = '{}/{}'.format(self.viz_outpath, train_mode)
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
            print('dir created {}'.format(save_dir))
