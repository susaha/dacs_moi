...
from ctrl.panoptic_deeplab_transforms.build import build_transforms
...
class CityscapesDataSet(data.Dataset):
    ...
    def __init__(self, root, list_path, set='val', max_iters=None, crop_size=(512, 1024),
        ...
        if self.set == 'train' and self.cfg.DACS_RANDOM_CROP:
            self.trs = self.cfg.PANOPTIC_DEEPLAB_TRANSFORMS_RANDOM_SCALES  # transforms random scale , for diable [1, 1, 0]
            # this basically adds RandomScale, RandomCrop, RandomHorizontalFlip, I havenot changed the name self.dacs_random_crop for coding conveniene
            crop_h = self.cfg.DATASET.RANDOM_CROP_DIM
            crop_w = self.cfg.DATASET.RANDOM_CROP_DIM
            self.dacs_random_crop = build_transforms(is_train=True, min_scale=self.trs[0], max_scale=self.trs[1], scale_step_size=self.trs[2], crop_h=crop_h, crop_w=crop_w)
    ...
    def __getitem__(self, index):
        ...
        if self.set == 'train':
            ...
            if self.cfg.DACS_RANDOM_CROP:
                ...
                image, label_panop, depth = self.dacs_random_crop(image, label_panop, use_depth=None, depth=None)
            ...
            if self.gen_panop_labels:
                ...
                semantic = np.zeros((image.shape[1], image.shape[2]), dtype=np.uint8)  # dummy semantic label as you don't need it during UDA training
    ...
    def visualize(self, name, image, semantic, center, center_w, offset, offset_w, visdepth=False, depth=None, mode=None, train_mode=None,  foreground=None, semantic_weights=None):
        save_dir = '/media/suman/DATADISK2/apps/experiments/CVPR2022/cityscapes_panoptic_gt_label_visualization_Jan_15/cityscapes_panop_jan31_2022/{}'.format(train_mode)
