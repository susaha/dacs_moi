...
class SYNTHIADataSetDepth(data.Dataset):
    def __init__(
        ...
        self.logger.info('*****************************************************************************************')
        self.logger.info('ctrl/dataset/synthia_panop_jan04_2022_v4.py -->  class SYNTHIADataSetDepth() : __init__()')
        self.logger.info(' *** LUKAS RARE CLS SAMPLINING INCLUDED *** ')
        self.logger.info('*****************************************************************************************')
        ...
        if num_classes == 16:
            ...
            self.map_19_to_16 = {}
            self.map_19_to_16[0] = [0, 'Road']  # road
            self.map_19_to_16[1] = [1, 'Sidewalk']  # Sidewalk
            self.map_19_to_16[2] = [2, 'Building']  # Building
            self.map_19_to_16[3] = [3, 'Wall']  # Wall
            self.map_19_to_16[4] = [4, 'Fence']  # Fence
            self.map_19_to_16[5] = [5, 'Pole']  # Pole
            self.map_19_to_16[6] = [6, 'TrafficLight']  # Traffic light
            self.map_19_to_16[7] = [7, 'TrafficSign']  # Traffic sign
            self.map_19_to_16[8] = [8, 'Vegetation']  # Vegetation
            self.map_19_to_16[10] = [9, 'Sky']  # sky
            self.map_19_to_16[11] = [10, 'Person']  # Pedestrian or person
            self.map_19_to_16[12] = [11, 'Rider']  # Rider
            self.map_19_to_16[13] = [12, 'Car']  # Car
            self.map_19_to_16[15] = [13, 'Bus']  # Bus
            self.map_19_to_16[17] = [14, 'Motorcycle']  # Motorcycle
            self.map_19_to_16[18] = [15, 'Bicycle']  # Bicycle
        ...
        self.get_semantic_label = SemanticTargetGenerator(self.ignore_label, self.rgb2id)
        ...
        for ann in dataset['annotations']:
            ...
            self.files[img_id] = (img_file, panop_label_file, segments_info, depth_file, name)

        # DAFormer rare class sampling
        if self.cfg.DEBUG:
            filename = '/media/suman/CVLHDD/apps/experiments/CVPR2022/Lukas_rare_cls_sampling/rcs_dict.pkl'
        else:
            filename = '/cluster/work/cvl/susaha/dataset/panoptic-datasets/rcs_dict.pkl'
        infile = open(filename, 'rb')
        rcs_dict = pickle.load(infile)
        infile.close()
        self.rcs_classes = rcs_dict['rcs_classes']
        self.rcs_classprob = rcs_dict['rcs_classprob']
        self.samples_with_class = rcs_dict['samples_with_class']
        # self.file_to_idx = rcs_dict['file_to_idx']
        self.rcs_min_crop_ratio = rcs_dict['rcs_min_crop_ratio']
        self.rcs_min_pixels = rcs_dict['rcs_min_pixels']
    ...
    def visualize(self, name, image, semantic, center, center_w, offset, offset_w, visdepth=False, depth=None):
        save_dir = '/media/suman/DATADISK2/apps/experiments/CVPR2022/synthia_panoptic_gt_label_visualization_Jan_05/synthia_panop_rare_cls_sample_feb07_2022/crowd-th-{}'.format(self.crowd_th)
    ...
    def __getitem__(self, index):
        ...
        # rare class samplining
        c = np.random.choice(self.rcs_classes, p=self.rcs_classprob)
        # print('*** sampled class : {} ***'.format(self.map_19_to_16[c][1]))
        f1 = np.random.choice(self.samples_with_class[c])
        file_name = f1.split('_')[0]
        img_file, panop_label_file, segment_info, depth_file, name = self.files[file_name]
        ...
        if self.cfg.DACS_RANDOM_CROP:
            ...
            for j in range(10):
                image_cropped, label_panop_cropped, depth_cropped = self.dacs_random_crop(image, label_panop, depth=depth, use_depth=self.use_depth, train_mode=True)
                label_semantic = self.get_semantic_label(label_panop_cropped, segment_info)
                n_class = torch.sum(label_semantic['semantic'] == self.map_19_to_16[c][0]) # Lukas trainIds are 0 to19 and I use 0 to 16, so need to map the Lukas trainId to my trainid
                if n_class > self.rcs_min_pixels * self.rcs_min_crop_ratio:
                    break
            image = image_cropped
            image = image.transpose((2, 0, 1))
            label_panop = label_panop_cropped
            depth = depth_cropped
        ...
        if not self.use_depth:
            return image, semantic.copy(), center.copy(), center_w.copy(), offset.copy(), offset_w.copy(), instance.copy(), shape, name
        else:
            return image, semantic.copy(), center.copy(), center_w.copy(), offset.copy(), offset_w.copy(), depth.copy(), instance.copy(), shape, name
