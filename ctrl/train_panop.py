...
def train_model(cfg, model, discriminator, discriminator2nd, resume_iteration,
    ...
    try:
        for i_iter in range(start_iter, max_iter):
            ...
            if not cfg.TRAIN_ONLY_SOURCE and cfg.ENABLE_DISCRIMINATOR:
                ...
                if cfg.APPROACH_TYPE == 'DANDA' and ((i_iter + 1) % cfg.TRAIN_DISC_EVERY) == 0:
                    for param in discriminator.parameters():
                        param.requires_grad = True
                elif cfg.APPROACH_TYPE != 'DANDA':
                    for param in discriminator.parameters():
                        param.requires_grad = True
                ...
                if (cfg.APPROACH_TYPE == 'DANDA' and ((i_iter + 1) % cfg.TRAIN_DISC_EVERY) == 0) or cfg.APPROACH_TYPE != 'DANDA':
                    loss_d1st = criterion_disc(d_out, source_label)
                    print('update disc1: {}'.format(i_iter))
                else:
                    loss_d1st = 0
                ...
                if (cfg.APPROACH_TYPE == 'DANDA' and ((i_iter + 1) % cfg.TRAIN_DISC_EVERY) == 0) or cfg.APPROACH_TYPE != 'DANDA':
                    ...
                    loss_d1st = 0
                    loss_meter_dict['loss_disc'].update(loss_d1st, batch_size)
