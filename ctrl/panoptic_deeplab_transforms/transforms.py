...
class Compose(object):
    ...
    def __call__(self, image, label, use_depth=None, depth=None):
        for t in self.transforms:
            image, label, depth = t(image, label, use_depth=use_depth, depth=depth)
...
class RandomScale(object):
    ...
    def __call__(self, image, label, use_depth=None, depth=None):
        ...
        image = cv2.resize(image.astype(np.float64), None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_LINEAR)
        label = cv2.resize(label.astype(np.float64), None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_NEAREST)
        if use_depth:
            depth_aug = cv2.resize(depth.astype(np.float64), None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_NEAREST)
            depth_aug.astype(depth_dtype)
        return image.astype(image_dtype), label.astype(label_dtype), depth_aug
...
class RandomCrop(object):
    ...
    def __call__(self, image, label, use_depth=None, depth=None):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            if use_depth:
                depth_pad = cv2.copyMakeBorder(depth, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_CONSTANT,
                                           value=1.0)  # inverse depth, 1 denotes the maximum depth 65536.0
        else:
        ...
        if use_depth:
            depth_aug = np.asarray(depth_pad[h_off:h_off + self.crop_h, w_off:w_off + self.crop_w], np.float32)
            depth_aug = depth_aug.astype(depth_dtype)
        return image.astype(image_dtype), label.astype(label_dtype), depth_aug
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, image, label, use_depth=None, depth=None):
        ...
        if random.random() < self.prob:
            ...
            label = label[:, ::-1].copy()
            if use_depth:
                depth_aug = depth[:, ::-1].copy()
            return image, label, depth_aug
        else:
            return image, label, depth
