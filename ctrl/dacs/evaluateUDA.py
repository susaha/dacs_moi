...
from ctrl.dacs.data import get_data_path, get_loader
import torchvision.transforms as transform

from PIL import Image
import scipy.misc
from ctrl.dacs.utils.loss import CrossEntropy2d
...
def get_iou(data_list, class_num, dataset, save_path=None):
    ...
    from ctrl.dacs.utils.metric import ConfusionMatrix
    ...
    for i, iou in enumerate(j_list):
        if i not in [9,  14, 16]:
            logger.info('class {:2d} {:12} IU {:.2f}'.format(i, classes[i], 100 * j_list[i]))
            mean_iou.append(100 * j_list[i])

    mean_iou = np.mean(mean_iou)
    logger.info('meanIOU: {:.4f}'.format(mean_iou))
    # logger.info('meanIOU: ' + str(aveJ) + '\n')
    ...
    return mean_iou, j_list
    # return aveJ, j_list

def evaluate(model, data_dir_source, data_dir_target, dataset, ignore_label=250,
             save_output_images=False, save_dir=None, input_size=(512,1024), debug=False):

    if dataset == 'cityscapes':
        num_classes = 19
        data_loader = get_loader('cityscapes')
        data_path = get_data_path('cityscapes', data_dir_source, data_dir_target)
        test_dataset = data_loader( data_path, img_size=input_size, img_mean = IMG_MEAN,
        ...
        data_path = get_data_path('gta', data_dir_source, data_dir_target)
    ...
    logger.info('Evaluating, found ' + str(len(testloader)) + ' images.')
    ...
    for index, batch in enumerate(testloader):
        ...
        with torch.no_grad():
            ...
            if dataset == 'cityscapes':
                gt = np.asarray(label[0].numpy(), dtype=np.int64)
            elif dataset == 'gta':
                gt = np.asarray(label[0].numpy(), dtype=np.int64)
        ...
        if (index+1) % 100 == 0:
            logger.info('%d processed'%(index+1))

        if index > 10 and debug == True:
            break
