...
from ctrl.dacs.data.city_utils import recursive_glob
...
class cityscapesLoader(data.Dataset):
    ...
    def transform(self, img, lbl):
        ...
        img = np.array(Image.fromarray(img).resize((self.img_size[0], self.img_size[1]), Image.BICUBIC)) # (width, height)
        img = img[:, :, ::-1]  # RGB -> BGR
        ...
        lbl = np.array(Image.fromarray(lbl).resize((self.img_size[0], self.img_size[1]), Image.NEAREST)) # (width, height)
