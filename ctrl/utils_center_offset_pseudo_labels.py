...
class CenterAndOffsetTargetGenerator(object):
    def __init__(self, logger, sigma=8, device=None):
        self.logger = logger
...
def get_instance_segmentation(semantic_prediction, ctr_hmp, offsets, thing_list, coord, threshold=0.1, nms_kernel=3, top_k=None, target_instance_id_offsets=None):
    ...
    return instance_prediction, center_weights, offset_weights, NO_VALID_SEGMENT
...
# if there is no valid center predicted then return tensor with all zeros
#     if ctr.size(0) == 0:
#         NO_VALID_CENTER = True
#         return torch.zeros_like(semantic_prediction), ctr.unsqueeze(0), None, None, NO_VALID_CENTER

'''
after multiplyining the thing_seg, ins_seg might have only one segment with id 0
for those case where instance_prediction[0].unique() returns [0] we need to set NO_VALID_CENTER=True
Now it might be the case that for one image I have valid center and for other img I don't have valid center
We need to have a NO_VALID_CENTER = [True, False], something like this
So that still if we have valid center for one image we can backprop
'''

# def find_instance_center(ctr_hmp, threshold=0.1, nms_kernel=3, top_k=None):
#     if ctr_hmp.size(0) != 2:
#         raise ValueError('Only supports batch size = 2')
#
#     ctr_hmp = F.threshold(ctr_hmp, threshold, -1)
#     # NMS
#     nms_padding = int((nms_kernel - 1) // 2)
#     ctr_hmp_max_pooled = F.max_pool2d(ctr_hmp, kernel_size=nms_kernel, stride=1, padding=nms_padding) # ctr_hmp_max_pooled: tensor(1,1,1024,2048)
...
#     ctr_all = torch.nonzero(ctr_hmp > 0) # ctr_all: tensor (N, 3), N is the no of center > 0
#     if top_k is None: # False
#         return ctr_all
#     elif ctr_all.size(0) < top_k: # if we have number of predicted centers less tan top_k (i.e. 200) then return all centers
#         return ctr_all
#     else:   # if we have number of predicted centers greater tan top_k (i.e. 200) then pick 200 top centers
#         # ctr_hmp: tensor(1024,2048), top_k=200, torch.flatten(ctr_hmp) gives you a tensor of shape 1024x2048,
#         # top_k_scores: tensor(k,) having scores in descending order, the top scoreis the first element e.g. 0.94, 0.92, 0.89
#         batch_size = ctr_hmp.size(0)
#         ctr_all = torch.zeros((top_k-1)*batch_size, 3)
#
#         # TODO: the following block is hard coded to support only batch siye 2, that is there are only batch id 0 and 1
#         ctr_all[top_k-1:,0] = 1 # assigning batch id for the 2nd batch
#         for bid in range(batch_size):
#             top_k_scores, _ = torch.topk(torch.flatten(ctr_hmp[bid, :]), top_k)
#             if bid == 0:
#                 ctr_all[:top_k-1, 1:] = torch.nonzero(ctr_hmp[bid,:] > top_k_scores[-1]) # batch 1 centers : storing from 0 to top_k-1 column
#             else:
#                 ctr_all[top_k-1:, 1:] = torch.nonzero(ctr_hmp[bid, :] > top_k_scores[-1]) # batch 1 centers : storing from top_k to top_k*2-1 column
#         return ctr_all