...
class DACSPanopDF(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.TRAIN.TRAIN_DEPTH_BRANCH:
            if self.cfg.DADA_STYLE_DEPTH_HEAD:
        ...
        if self.cfg.TRAIN.TRAIN_DEPTH_BRANCH:
            if not self.cfg.DADA_STYLE_DEPTH_HEAD:
    ...
    def forward(self, x, domain_label=0):
        ...
        if self.cfg.TRAIN.TRAIN_DEPTH_BRANCH and self.cfg.INST_DEPTH_FUSION_MODE == 2:
            pred['semantic'] = self.semantic_head(dec_out * pred['depth'])
        else:
