...
class DACSPanopDF(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.logger.info('ctrl/dacs_old/model/dacs_panop_model_v3.py --> class DACSPanopDF -->  def __init__()')
        self.cfg = cfg
        ...
        self.depth_head_src = DecFinalClassifier(1, 2048)
        ...
        self._init_params(self.depth_head_src)
    ...
    def forward(self, x, domain_label=0):
        ...
        pred['D_src'] = self.depth_head_src(dec_out)
        if self.cfg.INST_DEPTH_FUSION_MODE == 1:
            ...
            inst_x = self.instance_head(dec_out * pred['D_src'])  # fusining the depth pred with the input feature to the instance deeplab decoder
            pred['center'] = self.center_sub_head(inst_x)
            pred['offset'] = self.offset_sub_head(inst_x)
        elif self.cfg.INST_DEPTH_FUSION_MODE == 3:
            inst_x = self.instance_head(dec_out * ((pred['depth'] + pred['D_src']) * 0.5))  # fusining the depth pred with the input feature to the instance deeplab decoder
            pred['center'] = self.center_sub_head(inst_x)
            pred['offset'] = self.offset_sub_head(inst_x)
