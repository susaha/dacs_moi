...
class DACSPanopDF(nn.Module):

    def __init__(self, cfg):
        super(DACSPanopDF, self).__init__()

        self.logger = logging.getLogger(__name__)

        self.logger.info('ctrl/dacs_old/model/dacs_panop_model_v2.py --> class DACSPanopDF -->  def __init__()')
        self.cfg = cfg
        ...
        # Depth encoder
        self.depth_head = DecFinalClassifier(1, 2048)
    ...
    def forward(self, x, domain_label=0):
        ...
        dec_out = x * dec_out
        pred['depth'] = self.depth_head(dec_out)
        if self.cfg.INST_DEPTH_FUSION_MODE == 1:
            pred['semantic'] = self.semantic_head(dec_out * pred['depth'])  # depth fusion with semantic
            inst_x = self.instance_head(dec_out * pred['depth'])            # depth fusion with instance
            pred['center'] = self.center_sub_head(inst_x)
            pred['offset'] = self.offset_sub_head(inst_x)
        elif self.cfg.INST_DEPTH_FUSION_MODE == 2:
            pred['semantic'] = self.semantic_head(dec_out * pred['depth'])  # depth fusion with semantic onlz gives the best results
            inst_x = self.instance_head(dec_out)
            pred['center'] = self.center_sub_head(inst_x)
            pred['offset'] = self.offset_sub_head(inst_x)
        elif self.cfg.INST_DEPTH_FUSION_MODE == 3:
            pred['semantic'] = self.semantic_head(dec_out)
            inst_x = self.instance_head(dec_out * pred['depth'])
            pred['center'] = self.center_sub_head(inst_x)
            pred['offset'] = self.offset_sub_head(inst_x)
        elif self.cfg.INST_DEPTH_FUSION_MODE == 4:
            dec_out = dec_out * pred['depth']
            pred['semantic'] = self.semantic_head(dec_out)
            inst_x = self.instance_head(dec_out)
            pred['center'] = self.center_sub_head(inst_x)
            pred['offset'] = self.offset_sub_head(inst_x)
    ...
    def forward_with_backbone_feat(self, x, domain_label=0):
        pred = OrderedDict()
        x = self.backbone(x)
        enc_out = self.dada_aux_encoder(x)
        dec_out = self.dada_aux_decoder(enc_out)
    ...
    # def forward_old(self, x, domain_label=0):
    #     pred = OrderedDict()
    #     x = self.backbone(x)
    #     enc_out = self.dada_aux_encoder(x)
    #     dec_out = self.dada_aux_decoder(enc_out)
    ...
    #     pred['depth'] = self.depth_head(dec_out)
    #     if self.cfg.INST_DEPTH_FUSION_MODE == 1:
    #         inst_x = self.instance_head(dec_out * pred['depth'])  # fusining the depth pred with the input feature to the instance deeplab decoder
    #         pred['center'] = self.center_sub_head(inst_x)
    #         pred['offset'] = self.offset_sub_head(inst_x)
    #     elif self.cfg.INST_DEPTH_FUSION_MODE == 2:
    #         inst_x = self.instance_head(dec_out)
    #         pred['center'] = self.center_sub_head(inst_x * pred['depth'])   # fusing the depth pred with the input feature to the center sub head
    #         pred['offset'] = self.offset_sub_head(inst_x)
    #     elif self.cfg.INST_DEPTH_FUSION_MODE == 3:
    #         inst_x = self.instance_head(dec_out)
    #         pred['center'] = self.center_sub_head(inst_x * pred['depth'])   # fusing the depth pred with the input feature to the center sub head
    #         pred['offset'] = self.offset_sub_head(inst_x * pred['depth'])   # fusing the depth pred with the input feature to the center and offset sub head
    #     elif self.cfg.INST_DEPTH_FUSION_MODE == 4:
    #         inst_x = self.instance_head(dec_out)
    #         pred['center'] = self.center_sub_head(inst_x) * pred['depth']
    #         pred['offset'] = self.offset_sub_head(inst_x)
    #     elif self.cfg.INST_DEPTH_FUSION_MODE == 5:
    #         inst_x = self.instance_head(dec_out)
    #         pred['center'] = self.center_sub_head(inst_x) * pred['depth']
    #         pred['offset'] = self.offset_sub_head(inst_x) * pred['depth']
    #     return pred
