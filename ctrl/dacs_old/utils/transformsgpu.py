...
def oneMix_modi(mask, data = None, target = None):
    #Mix
    if not (data is None):
        stackedMask0, _ = torch.broadcast_tensors(mask[0], data[0])
        data = (stackedMask0*data[0]+(1-stackedMask0)*data[1]).unsqueeze(0)
    if not (target is None):
        stackedMask0, stackedMask1 = torch.broadcast_tensors(mask[0], target[0])

        img1 = stackedMask0*target[0]

        im = Image.fromarray((stackedMask0.squeeze().detach().clone().cpu().numpy()*250), 'F')  # stackedMask0 has value 0 and 1
        im.show()
        im = Image.fromarray((stackedMask1.squeeze().detach().clone().cpu().numpy()*250), 'F') # stackedMask0 has value float between 0 and 1
        im.show()
        im = Image.fromarray((target[0].squeeze().detach().clone().cpu().numpy()*250), 'F')
        im.show()
        im = Image.fromarray((img1.squeeze().detach().clone().cpu().numpy()*250), 'F')
        im.show()
        print()

        # img2 = (1 - stackedMask0) * target[1]

        # im = Image.fromarray(((1-stackedMask0).squeeze().detach().clone().cpu().numpy() * 250).astype(np.uint8))
        # im = im.convert('RGB')
        # im.show()

        # im = Image.fromarray((img2.squeeze().detach().clone().cpu().numpy() * 250).astype(np.uint8))
        # im = im.convert('RGB')
        # im.show()
        # im = Image.fromarray((target.squeeze().detach().clone().cpu().numpy() * 250).astype(np.uint8))
        # im = im.convert('RGB')
        # im.show()
