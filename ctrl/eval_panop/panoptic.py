...
class CityscapesPanopticEvaluator:
    ...
    def __init__(self, output_dir=None, train_id_to_eval_id=None, label_divisor=1000, void_label=255000,
        ...
        self._logger.info('ctrl/eval_panop/panoptic.py -->  def __init__() --> self._logger : {}'.format(self._logger))

        if 'Cityscapes' in target_dataset_name:
            self._gt_json_file = os.path.join(gt_dir, 'gtFineV2', panoptic_josn_file.format(split))
            self._gt_folder = os.path.join(gt_dir, 'gtFineV2', panoptic_json_folder.format(split))
    ...
    def evaluate(self):
        ...
        with contextlib.redirect_stdout(io.StringIO()):
            results = cityscapes_eval.evaluatePanoptic(gt_json_file, gt_folder, pred_json_file, pred_folder,
                                                       ...
                                                       input_image_size=self.input_image_size, mapillary_dataloading_style=self.mapillary_dataloading_style)
