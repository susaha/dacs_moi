...
def eval_panoptic(config, model, eval_folder, writer, i_iter, logger, device, data_loader, img_ids=None):
    ...
    pool_h = pool_h - 1 # to make it from 33 to 32 # panoptic_deeplab use a shape 1025 x 2049 at training time, we can use this same shape, but then we need to change the cityscapes evaluation script where it reads the panptic gt labels of shape 2048 x 1024
    pool_w = pool_w - 1 # to make it from 65 to 64
    # once eval is done then call model.set_image_pooling(); refer to class ASPPPooling(nn.Module) --> ctrl/model_panop/decoder/aspp.py
        ...
        with torch.no_grad():
            for i, batch in enumerate(data_loader):
                ...
                if fixed_test_size: # this is for cityscapes
                    ...
                    upsample_dim = (data['semantic'].shape[2], data['semantic'].shape[1])
