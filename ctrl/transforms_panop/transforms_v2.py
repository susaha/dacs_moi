...
class ToTensor(object):
    ...
    """
    def __call__(self, image, label, label2):
        return F.to_tensor(image), label, label2
...
class RandomScale(object):
    ...
    def __call__(self, image, label, label2):
        ...
        label2 = cv2.resize(label2.astype(np.float), None, fx=f_scale, fy=f_scale, interpolation=cv2.INTER_NEAREST)
...
class RandomCrop(object):
    ...
    def __init__(self, crop_h, crop_w, pad_value, ignore_label, random_pad):
        ...
        logger.info('ctrl/transforms_panop/transforms_v2.py --> class RandomCrop(...) : def __init__(...)')
    ...
    def __call__(self, image, label, label2, label_depth=None, crop_depth=None, train_mode=None):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            label2_pad = cv2.copyMakeBorder(label2, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_CONSTANT,
                                           value=self.ignore_label)
            label_depth_pad = cv2.copyMakeBorder(label_depth, pad_top, pad_bottom, pad_left, pad_right, cv2.BORDER_CONSTANT,
                                            value=0)
        else:
            if crop_depth:
