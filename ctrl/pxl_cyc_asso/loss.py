...
class AssociationLoss(nn.Module):
    ...
    def associate_gt(self, gt, indices, gt2=None, mode=0):
        N, H, W = gt.size()
        K = indices.size(2)
        gt = gt.view(N, -1, 1).expand(N, -1, K)
        if mode == 1:
            gt2 = gt2.view(N, -1, 1).expand(N, -1, K)
        ...
        if mode == 0:
            ...
            gt = ((gt == associated_gt) * (gt2 == associated_gt)).type(torch.cuda.FloatTensor).detach()
    ...
    def forward(self, x1, x2, gt1, gt2=None, pseudo_gt2=None, mode=0):
        gt1 = gt1.float()

        N, _, H, W = x1.size()
        _, _, H2, W2 = x2.size()
        assert (x1.shape[:2] == x2.shape[:2]), x2.size()
        ignore_mask = (gt1 == 255) # TODO cfg.DATASET.IGNORE_LABEL
        ...
        ass_gt = self.associate_gt(gt1, indices, gt2=pseudo_gt2, mode=mode)
