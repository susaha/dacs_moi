import torch
from ctrl.utils.common_config_panop import get_model, get_criterion, \
    get_optimizer_v2, get_data_loaders, get_optimizer_panop_v1, get_criterion_panop, get_optimizer_dada, get_optimizer_dacs
from time import perf_counter
from ctrl.utils.train_utils import print_output_paths, log_output_paths
import argparse
...
from ctrl.solver_panop import build_lr_scheduler, get_lr_group_id
...
from ctrl.utils.panoptic_deeplab.env import seed_all_rng  # TODO
from ctrl.dacs_old.model.dacs_model import DACSModel  # TODO
from ctrl.dacs_old.utils.train_uda_scripts import create_ema_model, create_ema_model_dacs_ori  # TODO
...
from ctrl.train_panop_utils import create_panop_eval_dirs
...
# for DACS training with semantic segmentation only
...
def parse_args():
    ...
    if config.DEBUG:
        ...
        # "SOLVER.BASE_LR", 0.00005, "LOSS.CENTER.WEIGHT", 200.0, "LOSS.OFFSET.WEIGHT", 0.01, "TRAIN.MAX_ITER", 21]

        # "TRAIN.IMGNET_PRETRAINED_MODEL_PATH", pimnet_path
...
def main():
    ...
    # UPSAMPLER
    interp = nn.Upsample(size=(input_shape_source[0], input_shape_source[1]), mode='bilinear', align_corners=True)
    ...
    try:
        for i_iter in range(start_iter, max_iter):
            ...
            if True: # cfg.ACTIVATE_SEMANITC_EVAL:
            ...
            if False: # cfg.ACTIVATE_PANOPTIC_EVAL:
                if (not cfg.DISTRIBUTED and ((i_iter + 1) % cfg.TRAIN.EVAL_EVERY == 0)) \
                        or (not cfg.DISTRIBUTED and ((i_iter + 1) == cfg.TRAIN.MAX_ITER)):
                    logger.info('')
